...
plt.rc("figure", figsize=[46.82 * 0.5 ** (0.5 * A), 35.61 * 0.5 ** (0.5 * A)])
...
t1 = time.time()
lambN = dr.find_eigenvalue(
    GAdj, maxIter=500, dt=0.01, checkStep=25
...
sigma, sigmaArray = dr.optimal_sigma(
    GAdj, analytical=analytical, endVal=lambN
)  # get the optimal sigma using the space (0, -1/lambN) as computed previously
...
ourDomiRankAttack = generate_attack(
    ourDomiRankDistribution
)  # generate the attack using the centrality (descending)
domiRankRobustness, domiRankLinks = network_attack_sampled(
